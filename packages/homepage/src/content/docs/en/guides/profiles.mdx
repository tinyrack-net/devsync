...
Tell devsync which profile to use on the current machine:

```bash
devsync profile use work
```

From now on, `devsync push` and `devsync pull` on this machine will include default entries **plus** entries assigned to the `work` profile.
...
This shows every profile name that appears in `manifest.json`, along with which one is currently active.
...
```bash
devsync profile use work
devsync push    # pushes default + work entries
```
...
devsync pull    # pulls default + personal entries only
```
...
<Aside type="note">
An entry assigned to multiple profiles (e.g., `--profiles work,personal`) is stored once in the sync directory and synced whenever either profile is active.
</Aside>
