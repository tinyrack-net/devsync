...
- **Repository validity** — whether `~/.config/devsync/repository/` exists and is a valid Git repository
- **Manifest integrity** — whether `manifest.json` is present and syntactically valid
- **Age key** — whether `~/.config/devsync/keys.txt` exists and is readable
...
**Cause:** `~/.config/devsync/keys.txt` is missing or has been corrupted.
...
**Cause:** `~/.config/devsync/repository/` was deleted or `devsync init` was never run on this machine.
