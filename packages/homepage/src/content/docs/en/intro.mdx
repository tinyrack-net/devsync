...
`Devsync` is a **CLI tool that syncs your development environment config files (dotfiles) with Git**. It's designed for developers who want to keep a consistent environment across multiple machines.

---

## What Devsync can do

- **Track files and directories**: Register files and folders in your home directory as sync targets with the `track` command.
...
There are trade-offs: Devsync doesn't have the powerful template-based file transformation that `chezmoi` offers. It's not designed for complex scenarios where config content needs to differ per machine — it's designed to **keep your editing workflow natural while syncing your config across devices**.
