...
`Devsync` is a **CLI tool that syncs your development environment config files (dotfiles) with Git**. Keep a consistent setup across multiple machines.
...
Devsync takes the opposite approach. **Your local files are always the source of truth.** The sync directory is just a mirror. Edit `~/.zshrc` directly in your favorite editor — no special commands needed. When you're ready to sync, run `devsync push` to copy your local files into the sync directory, then use Git to share them.
...
Head to [Getting Started](/en/getting-started/) to install devsync and set up your first sync.
