...
1. **Initialize devsync**
   ...
   You'll be asked if you want to provide an existing age private key. If this is your first time, just press Enter — a new key will be generated automatically.

   After initialization, the following structure is created under `~/.config/devsync/`:

   <FileTree>
   - ~/.config/devsync/
     - sync/          Git repository where synced files are stored
     - age/
   ...
   <Aside type="caution">
   `age/keys.txt` is the private key used to decrypt your secret files. If you lose it, encrypted files cannot be recovered. **Back it up somewhere safe.**
   </Aside>
...
Secret files are automatically decrypted using your age key.
