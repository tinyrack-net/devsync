...
<Aside type="note">
  After untracking, the artifact in the sync repository (`~/.config/devsync/sync/`) still exists. To remove it permanently, open the sync directory with [`devsync cd`](/en/reference/cd) and delete the file manually, then commit the change.
</Aside>
