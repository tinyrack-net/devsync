---
title: 시크릿 파일 동기화
description: age 암호화를 사용해 SSH 설정, API 토큰, 인증 정보 같은 민감한 파일을 안전하게 동기화해요.
---

import { Aside, Steps, FileTree } from "@astrojs/starlight/components";

일부 설정 파일에는 SSH 키, API 토큰, 인증 정보처럼 git 저장소에 평문으로 올리면 안 되는 민감한 데이터가 들어있어요. Devsync는 이런 파일을 위해 **secret 모드**를 제공해요. 저장 전에 [age](https://age-encryption.org/)로 암호화해요.

---

## secret 모드를 사용해야 할 때

저장소가 공개될 경우 노출되면 곤란한 파일에는 `--mode secret`을 써요.

- `~/.ssh/config` — SSH 호스트 설정
- `~/.aws/credentials` — AWS 액세스 키
- `~/.config/gh/hosts.yml` — GitHub CLI 토큰
- 인증 토큰이 포함된 `~/.npmrc`
- 실제 인증 정보가 담긴 `.env` 파일
- API 키, 비밀번호, 개인 토큰이 담긴 모든 파일

시크릿이 없는 파일(편집기 설정, 셸 alias, 토큰 없는 `.gitconfig`)은 기본 `normal` 모드를 사용하면 돼요.

---

## age 암호화 동작 방식

Devsync는 공개키 암호화 방식인 [age](https://age-encryption.org/)를 사용해요.

- **개인키** (`AGE-SECRET-KEY-...`): `~/.config/devsync/age/keys.txt`에 저장돼요
- **수신자 (공개키, `AGE1...`)**: 파일 암호화에 사용되는 공개키예요
- `push` 시: secret 파일이 등록된 모든 수신자의 공개키로 암호화돼요
- `pull` 시: 암호화된 파일이 내 개인키로 복호화돼요

이 방식의 특징:
- 개인키가 없으면 저장소에서 시크릿 파일 내용을 읽을 수 없어요
- 수신자를 여러 명 추가해서 여러 기기에서 시크릿을 공유할 수 있어요

---

## age 키 설정

age 키는 `devsync init` 시 자동으로 생성돼요. 처음 시작이라면 키 입력 프롬프트에서 엔터만 누르면 새 키 쌍이 만들어져요.

```bash
devsync init
# → 엔터를 눌러 새 age 키를 생성해요
```

개인키는 `~/.config/devsync/age/keys.txt`에 저장돼요.

<Aside type="caution">
**age 개인키를 반드시 백업해 두세요.** 키는 `AGE-SECRET-KEY-`로 시작하는 문자열이에요. 이 키를 잃어버리면 암호화된 파일을 복원할 수 없어요. 패스워드 매니저 같이 Devsync 외부의 안전한 곳에 보관해 두세요.
</Aside>

초기화 시 기존 키를 제공할 수도 있어요.

```bash
devsync init --key AGE-SECRET-KEY-1...
```

또는 키 파일 경로를 지정할 수도 있어요.

```bash
devsync init --identity ~/path/to/key.txt
```

---

## 시크릿 파일 추적하기

```bash
devsync track ~/.ssh/config --mode secret
devsync track ~/.aws/credentials --mode secret
```

push를 실행하면 암호화되어 저장소에 저장돼요.

```bash
devsync push
```

sync 디렉터리에는 `.devsync.secret` 확장자가 붙어서 저장돼요.

<FileTree>
- ~/.config/devsync/sync/default/
  - .ssh/
    - config.devsync.secret
  - .aws/
    - credentials.devsync.secret
</FileTree>

---

## 다른 기기에서 시크릿 파일 가져오기

<Steps>

1. 기존 저장소와 백업해 둔 개인키로 새 기기에서 Devsync를 초기화해요.

   ```bash
   devsync init https://github.com/yourname/dotfiles.git --key AGE-SECRET-KEY-1...
   ```

2. 파일을 가져와요.

   ```bash
   devsync pull
   ```

   시크릿 파일은 자동으로 복호화되어 로컬 경로에 저장돼요.

</Steps>

---

## 여러 기기를 위한 수신자 추가

기기마다 별도의 age 키 쌍을 사용한다면, 수신자를 추가해서 어느 기기에서도 복호화할 수 있게 할 수 있어요.

```bash
# 새 기기의 공개키(수신자)를 확인해요
cat ~/.config/devsync/age/keys.txt
# AGE-SECRET-KEY-... (개인키)
# 공개키는 init 시 출력되거나, 개인키에서 파생할 수 있어요

# 기존 기기에서 새 기기를 수신자로 추가해요
devsync init --recipient AGE1xyz...newdevicepublickey...
```

수신자를 추가하고 다시 push하면 시크릿 파일이 두 키 모두로 암호화돼요.

```bash
devsync push
```

<Aside type="note">
여러 수신자로 암호화된 파일은 각 수신자의 키로 독립적으로 복호화할 수 있어요. 이것은 age의 다중 수신자 암호화 기능이에요.
</Aside>

---

## 보안 유의사항

- 파일 **내용**은 암호화되지만, **파일 이름과 경로**는 저장소에 그대로 보여요
- `.devsync.secret` 확장자로 해당 파일이 추적된 시크릿임은 알 수 있어도, 내용은 알 수 없어요
- 공개 저장소에 시크릿 파일을 저장해도 내용 측면에서는 안전하지만, 경로 자체에서 설정 정보가 노출될 수 있다는 점을 고려하세요
