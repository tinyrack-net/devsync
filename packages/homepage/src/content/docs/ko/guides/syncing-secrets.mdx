...
- **Identity**는 비밀 키(`AGE-SECRET-KEY-...`)로, `~/.config/dotweave/age-identity.txt`에 저장돼요. `pull` 때 파일을 복호화하는 데 사용돼요.
...
비밀 키는 `~/.config/dotweave/age-identity.txt`에 저장되고, 대응하는 공개 키는 동기화 설정에 recipient로 등록돼요.
