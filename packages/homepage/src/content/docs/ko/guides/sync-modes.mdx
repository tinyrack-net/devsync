---
title: 동기화 모드
description: normal, secret, ignore 세 가지 동기화 모드와 각각의 사용 시점을 알아봐요.
---
...
파일을 [age](https://age-encryption.org/)로 암호화한 후 저장소에 저장해요. 저장된 아티팩트에는 `.devsync.secret` 확장자가 붙어요.
...
`pull` 시에는 암호화된 아티팩트가 age 개인키로 자동 복호화된 뒤 디스크에 저장돼요.
