---
title: 작동 방식
description: Devsync의 동작 원리와 내부 구조
---

import { Aside, FileTree, Steps } from "@astrojs/starlight/components";

Devsync의 핵심 철학은 하나예요: **진실의 원천은 항상 로컬 파일**이에요. 저장소는 동기화 사본일 뿐, 정본(canonical version)이 아니에요.
...
Devsync는 반대예요. `~/.zshrc`를 그냥 편집기로 열어서 수정하면 돼요 — 특별한 명령이 필요 없어요. 동기화할 준비가 되면 `devsync push`를 실행하면 현재 로컬 파일을 읽어 저장소에 복사해요.
...
매니페스트는 중앙 레지스트리예요. 추적 경로마다 소스 경로, 동기화 모드, 프로필 지정 정보가 담긴 항목이 들어있어요.
