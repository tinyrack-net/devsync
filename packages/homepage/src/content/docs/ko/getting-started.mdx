...
1. **저장소 초기화**
   ...
   <Aside type="caution">
   `keys.txt`는 시크릿 파일을 복호화하는 데 사용하는 개인키예요. 이 파일을 잃어버리면 암호화된 파일을 복원할 수 없으니 **안전한 곳에 별도로 백업해 두세요.**
   </Aside>
...
3. **저장소에 반영**
   ...
   로컬 파일들을 `~/.config/devsync/repository/` 안에 미러링해요. 이 시점에는 아직 git commit이 만들어지지 않아요.
